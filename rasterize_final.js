...
function handleKeyDown(event) {
    ...
    switch (event.code) {
        ...
        case "KeyB":
            ...
            toggle_Texture = (toggle_Texture + 1)%3;
        break;
    } // end switch
} // end handleKeyDown
...
//load ellipsoid texture
var ellipsoidTexture = [];
function loadEllipsoidTexture(textureLocation, ellipsoidSet)
{
    ...
    {
        handleTexture(ellipsoidTexture[ellipsoidSet]);
    }
    ellipsoidTexture[ellipsoidSet].image.onerror = function () {
...
//load ellipsoid texture
var triangleTexture = [];
function loadTriangleTexture(textureLocation, triangleSet)
{
    triangleTexture[triangleSet] = gl.createTexture();
    triangleTexture[triangleSet].image = new Image();
    triangleTexture[triangleSet].image.crossOrigin = "Anonymous";
    triangleTexture[triangleSet].image.onload = function()
    {
...
function handleTexture(textureVal)
{
    ...
    function makeEllipsoid(currEllipsoid,numLongSteps) {
        ...
        try {
            if (numLongSteps % 2 != 0)
                ...
                var ellipsoidUVs = [0,-1]; //south pole
                ...
                ellipsoidUVs.push(0,1); //north pole
                var latAngle =-latLimitAngle;
        ...
        if (inputTriangles == String.null)
            ...
            if (inputEllipsoids == String.null)
                ...
                for (var whichEllipsoid=0; whichEllipsoid<numEllipsoids; whichEllipsoid++) {
                    ...
                    //load ellipsoid texture
                    loadEllipsoidTexture(ellipsoid.texture, whichEllipsoid);
                    ...
                    //send ellipsoid uv coords to webgl
                    ...
                    {
                        //ellipoid is opaque
                        opaqueBuffer.push(ellipsoidSet);
                    }
                    else
                    {
                        //ellipsoid is transparent
                        transBuffer.push(ellipsoidSet);
                    }
...
function setupShaders() {
    ...
    var fShaderCode = `
        ...
        uniform int toggleTexture; //toggle texture value
        ...
        void main(void) {
            ...
            if(!uniformUseTexture)
            {
                ...
                else if (toggleTexture==2)
                {
                     //C = CfCt & A = At; 
                     gl_FragColor = vec4(colorOut, 1.0)*texelColor; 
                }
                else
                {
                     //C = CfCt & A = AfAt
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                uniToggleTexture = gl.getUniformLocation(shaderProgram, "toggleTexture");
...
function renderModels() {
    ...
    for(var opaqueSet =0; opaqueSet<opaqueBuffer.length; opaqueSet++)
    {
        if(opaqueBuffer[opaqueSet].model=="triangle")
        {
            ...
            gl.uniform3fv(ambientULoc,currOpaqueSet.detail.material.ambient); // pass in the ambient reflectivity
            gl.uniform3fv(diffuseULoc,currOpaqueSet.detail.material.diffuse); // pass in the diffuse reflectivity
            gl.uniform3fv(specularULoc,currOpaqueSet.detail.material.specular); // pass in the specular reflectivity
            gl.uniform1f(shininessULoc,currOpaqueSet.detail.material.n); // pass in the specular exponent
            gl.uniform1f(alphaULoc,currOpaqueSet.detail.material.alpha); //alpha location

            gl.uniform1f(useTextureLoc, currOpaqueSet.detail.material.texture); //use texture

            // vertex buffer: activate and feed into vertex shader
            gl.bindBuffer(gl.ARRAY_BUFFER,vertexBuffers[currOpaqueSet.set_number]); // activate
            gl.vertexAttribPointer(vPosAttribLoc,3,gl.FLOAT,false,0,0); // feed

            gl.bindBuffer(gl.ARRAY_BUFFER,normalBuffers[currOpaqueSet.set_number]); // activate
            gl.vertexAttribPointer(vNormAttribLoc,3,gl.FLOAT,false,0,0); // feed

            //uv buffer
            gl.bindBuffer(gl.ARRAY_BUFFER, textureBuffers[currOpaqueSet.set_number]);
            gl.vertexAttribPointer(uvTextureLoc,2,gl.FLOAT,false,0,0);

            //bind texture
            gl.activeTexture(gl.TEXTURE0);
            gl.bindTexture(gl.TEXTURE_2D, triangleTexture[currOpaqueSet.set_number]);
            gl.uniform1i(samplerUniform, 0);

            gl.uniform1i(uniToggleTexture, toggle_Texture);

            // triangle buffer: activate and render
            gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,triangleBuffers[currOpaqueSet.set_number]); // activate
            gl.drawElements(gl.TRIANGLES,3*triSetSizes[currOpaqueSet.set_number],gl.UNSIGNED_SHORT,0); // render
        }
        else if(opaqueBuffer[opaqueSet].model=="ellipsoid")
        {
         ...
         //   console.log("rendering opaque ellipsoid");
            ...
            // reflectivity: feed to the fragment shader
            gl.uniform3fv(ambientULoc,ellipsoid.detail.ambient); // pass in the ambient reflectivity
            gl.uniform3fv(diffuseULoc,ellipsoid.detail.diffuse); // pass in the diffuse reflectivity
            gl.uniform3fv(specularULoc,ellipsoid.detail.specular); // pass in the specular reflectivity
            gl.uniform1f(shininessULoc,ellipsoid.detail.n); // pass in the specular exponent
            gl.uniform1f(alphaULoc,ellipsoid.detail.alpha);
            gl.uniform1f(useTextureLoc, ellipsoid.detail.texture); //use texture

          //  console.log(" ************* ellipsoid.set_number:" + ellipsoid.set_number);

            gl.bindBuffer(gl.ARRAY_BUFFER,vertexBuffers[numTriangleSets+ellipsoid.set_number]); // activate vertex buffer
            gl.vertexAttribPointer(vPosAttribLoc,3,gl.FLOAT,false,0,0); // feed vertex buffer to shader
        
            gl.bindBuffer(gl.ARRAY_BUFFER,normalBuffers[numTriangleSets+ellipsoid.set_number]); // activate normal buffer
            gl.vertexAttribPointer(vNormAttribLoc,3,gl.FLOAT,false,0,0); // feed normal buffer to shader
        
            //bind texture buffer
            gl.bindBuffer(gl.ARRAY_BUFFER, textureBuffers[numTriangleSets+ellipsoid.set_number]);
            gl.vertexAttribPointer(uvTextureLoc,2,gl.FLOAT,false,0,0);

            gl.activeTexture(gl.TEXTURE0);
            gl.bindTexture(gl.TEXTURE_2D, ellipsoidTexture[ellipsoid.set_number]);
            gl.uniform1i(samplerUniform, 0);

            gl.uniform1i(uniToggleTexture, toggle_Texture);

            gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,triangleBuffers[numTriangleSets+ellipsoid.set_number]); // activate tri buffer
        
            // draw a transformed instance of the ellipsoid
            gl.drawElements(gl.TRIANGLES,triSetSizes[numTriangleSets+ellipsoid.set_number],gl.UNSIGNED_SHORT,0); // render
        }
    }
    ...
    for(var transSet =transBuffer.length-1; transSet>=0; transSet--)
    {
        ...
        if(transBuffer[transSet].model=="triangle")
        {
            ...
            // reflectivity: feed to the fragment shader
            gl.uniform3fv(ambientULoc,currTransSet.detail.material.ambient); // pass in the ambient reflectivity
            gl.uniform3fv(diffuseULoc,currTransSet.detail.material.diffuse); // pass in the diffuse reflectivity
            gl.uniform3fv(specularULoc,currTransSet.detail.material.specular); // pass in the specular reflectivity
            gl.uniform1f(shininessULoc,currTransSet.detail.material.n); // pass in the specular exponent
            gl.uniform1f(alphaULoc,currTransSet.detail.material.alpha); //alpha location
            ...
            gl.bindBuffer(gl.ARRAY_BUFFER,normalBuffers[currTransSet.set_number]); // activate
            gl.vertexAttribPointer(vNormAttribLoc,3,gl.FLOAT,false,0,0); // feed

            //uv buffer
            gl.bindBuffer(gl.ARRAY_BUFFER, textureBuffers[currTransSet.set_number]);
            gl.vertexAttribPointer(uvTextureLoc,2,gl.FLOAT,false,0,0);

            //bind texture
            gl.activeTexture(gl.TEXTURE0);
            gl.bindTexture(gl.TEXTURE_2D, triangleTexture[currTransSet.set_number]);
            gl.uniform1i(samplerUniform, 0);
        ...
        else if(transBuffer[transSet].model=="ellipsoid")
        {
            //render transparent ellipsoid
            ...
            // reflectivity: feed to the fragment shader
            gl.uniform3fv(ambientULoc,transEllipsoid.detail.ambient); // pass in the ambient reflectivity
            gl.uniform3fv(diffuseULoc,transEllipsoid.detail.diffuse); // pass in the diffuse reflectivity
            gl.uniform3fv(specularULoc,transEllipsoid.detail.specular); // pass in the specular reflectivity
            ...
            gl.bindBuffer(gl.ARRAY_BUFFER,normalBuffers[numTriangleSets+transEllipsoid.set_number]); // activate normal buffer
            gl.vertexAttribPointer(vNormAttribLoc,3,gl.FLOAT,false,0,0); // feed normal buffer to shader
        
            //bind texture buffer
            gl.bindBuffer(gl.ARRAY_BUFFER, textureBuffers[numTriangleSets+transEllipsoid.set_number]);
            gl.vertexAttribPointer(uvTextureLoc,2,gl.FLOAT,false,0,0);

            gl.activeTexture(gl.TEXTURE0);
            gl.bindTexture(gl.TEXTURE_2D, ellipsoidTexture[transEllipsoid.set_number]);
            gl.uniform1i(samplerUniform, 0);

            gl.uniform1i(uniToggleTexture, toggle_Texture);

            gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,triangleBuffers[numTriangleSets+transEllipsoid.set_number]); // activate tri buffer
        
            // draw a transformed instance of the ellipsoid
            gl.drawElements(gl.TRIANGLES,triSetSizes[numTriangleSets+transEllipsoid.set_number],gl.UNSIGNED_SHORT,0); // render
        }
     }
} // end render model
